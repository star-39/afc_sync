...
void init_pull_afc(afc_client_t afc, char* src, char* dst)
{
    ...
    if (src == NULL || dst == NULL) {
        printf("No src or dst specified!\n");
        return;
    }
    ...
    char** envp) // Array of environment variable strings)
{
    ...
    for (int c = 0; c < argc; c++) {
        if (!strcmp(argv[c], "--src")) {
            ...
            printf("afc_sync --src [SOURCE_DIR] --dst [DEST_DIR] --udid [UDID] --skip-exist --max-skips [MAX_SKIPS]\n\n");
            return 0;
        }
    }
    ...
    printf("Using %s\n", options.udid);
    if (idevice_new_with_options(&device, options.udid, IDEVICE_LOOKUP_NETWORK | IDEVICE_LOOKUP_USBMUX) != IDEVICE_E_SUCCESS) {
